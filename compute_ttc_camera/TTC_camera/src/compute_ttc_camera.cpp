                      ...
                      std::vector<cv::DMatch> kptMatches, double frameRate, double &TTC)
{
    ...
    int sizeDistRatios = distRatios.size();
    double medianDistRatio;

    if (sizeDistRatios%2 == 0)
        medianDistRatio = (distRatios[sizeDistRatios/2] +distRatios[sizeDistRatios/2 +1])/2;
    else
        medianDistRatio = distRatios[(int)sizeDistRatios/2 + 1];
