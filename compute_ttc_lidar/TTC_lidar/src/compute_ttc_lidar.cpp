                     ...
                     std::vector<LidarPoint> &lidarPointsCurr, double &TTC)
{
    ...
    double laneWidth = 4.0; // assumed width of the ego lane
  	...
  	std::vector<LidarPoint> lidarPointsPrevWidth, lidarPointsCurrWidth;
  
  	for (auto it = lidarPointsPrev.begin(); it != lidarPointsPrev.end(); ++it)
    {
        if (abs(it->y) < laneWidth/2)
        	lidarPointsPrevWidth.push_back(*it);
    }

    for (auto it = lidarPointsCurr.begin(); it != lidarPointsCurr.end(); ++it)
    {
        if (abs(it->y) < laneWidth/2)
        	lidarPointsCurrWidth.push_back(*it);
    }

    // find closest distance to Lidar points within ego lane
    double minXPrev = 1e9, minXCurr = 1e9;
    for (auto it = lidarPointsPrevWidth.begin(); it != lidarPointsPrevWidth.end(); ++it)
    {
        minXPrev = minXPrev > it->x ? it->x : minXPrev;
    }

    for (auto it = lidarPointsCurrWidth.begin(); it != lidarPointsCurrWidth.end(); ++it)
    {
